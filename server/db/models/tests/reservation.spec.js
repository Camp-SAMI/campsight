...
describe('Reservation model', () => {
    ...
    it('requires a startTime', async () => {
        const reservation = Reservation.build({endTime: Date.now()+3, camperId: camper.id, campsiteId: campsite.id});
        try {
            await reservation.validate();
            throw Error('validation is successful, but should have failed without `startTime`');
        } catch (err) {
            expect(err.message).to.contain('startTime cannot be null');
        }
    });
    it('requires an endTime', async () => {
        const reservation = Reservation.build({startTime: Date.now()+1, camperId: camper.id, campsiteId: campsite.id });
        try {
            await reservation.validate();
            throw Error('validation is successful, but should have failed without `endTime`')
        } catch (err) {
